...
void AudioInterfaceProvider::initMessageQueue()
{
    ...
    printf("AudioInterface: request feedback queue init. queueId: %d\n",feedbackQueueId);
...
bool AudioInterfaceProvider::sendMessageForResult(long int msgType,control_message *message,int intValue,char *textValue)
{
    m_mtx.lock();
